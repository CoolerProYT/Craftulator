...
import { itemIcon, itemName } from '../craftulator'
...
<template>
  <span class="cc-item" :class="{ 'with-label': label }">
    <span class="cc-slot" :title="name" :aria-label="name" role="img">
      <img v-if="src && !failed" class="pixelated" :src="src" alt="" loading="lazy" @error="failed = true" />
      <span v-else-if="id" class="cc-initials">{{ initials }}</span>
      <span v-if="count > 1" class="cc-count">{{ count }}</span>
    </span>
    <span v-if="label && id" class="cc-label">{{ name }}</span>
  </span>
</template>
...
.cc-slot {
  ...
  height: 36px;
  flex: none;
  background: var(--cc-slot-bg);
  border: 2px solid;
  border-color: var(--cc-slot-dark) var(--cc-slot-light) var(--cc-slot-light) var(--cc-slot-dark);
}
